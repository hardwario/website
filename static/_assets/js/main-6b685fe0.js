      ...
      $(window).resize(function(){
        ...
        if($('.submenu').length>0 && $(window).width()>1199) {
            ...
            $(window).scroll(function () {
                ...
                var hMin = $('#events').offset().top-100;
                var hMax = parseInt($('#events').offset().top)+parseInt($('#events').outerHeight())-100;
                if($(window).scrollTop()>=hMin && $(window).scrollTop()<=hMax)
                {
                    $('.submenu__link--events').addClass('active');
                }
                else
                {
                    $('.submenu__link--events').removeClass('active');
                }

                var hMin = $('#tv').offset().top-100;
                var hMax = parseInt($('#tv').offset().top)+parseInt($('#tv').outerHeight())-100;
                if($(window).scrollTop()>=hMin && $(window).scrollTop()<=hMax)
                {
                    $('.submenu__link--tv').addClass('active');
                }
                else
                {
                    $('.submenu__link--tv').removeClass('active');
                }

                var hMin = $('#articles').offset().top-100;
                var hMax = parseInt($('#articles').offset().top)+parseInt($('#articles').outerHeight())-100;
                if($(window).scrollTop()>=hMin && $(window).scrollTop()<=hMax)
                {
                    $('.submenu__link--articles').addClass('active');
                }
                else
                {
                    $('.submenu__link--articles').removeClass('active');
                }

                var hMin = $('#resources').offset().top-100;
                var hMax = parseInt($('#resources').offset().top)+parseInt($('#resources').outerHeight())-100;
                if($(window).scrollTop()>=hMin && $(window).scrollTop()<=hMax)
                {
                    $('.submenu__link--resources').addClass('active');
                }
                else
                {
                    $('.submenu__link--resources').removeClass('active');
                }
